...
class lane_controller(object):
    def __init__(self):
        ...
        self.sub_obstacle_avoidance_pose = rospy.Subscriber("~obstacle_avoidance_pose", LanePose, self.PoseHandling,
                                                            "obstacle_avoidance", queue_size=1)
        self.sub_obstacle_detected = rospy.Subscriber("~obstacle_detected", BoolStamped, self.setFlag,
                                                      "obstacle_detected", queue_size=1)

        # TO DO find node/topic in their branch
        self.sub_intersection_navigation_pose = rospy.Subscriber("~intersection_navigation_pose", LanePose,
                                                                 self.PoseHandling, "intersection_navigation",
                                                                 queue_size=1)  # TODO: remap topic in file catkin_ws/src/70-convenience-packages/duckietown_demos/launch/master.launch !
        ...
        self.sub_wheels_cmd_executed = rospy.Subscriber("~wheels_cmd_executed", WheelsCmdStamped,
                                                        self.updateWheelsCmdExecuted, queue_size=1)
        self.sub_actuator_limits = rospy.Subscriber("~actuator_limits", Twist2DStamped, self.updateActuatorLimits,
                                                    queue_size=1)

        # FSM
        self.sub_switch = rospy.Subscriber("~switch", BoolStamped, self.cbSwitch,
                                           queue_size=1)  # for this topic, no remapping is required, since it is directly defined in the namespace lane_controller_node by the fsm_node (via it's default.yaml file)
    ...
    def setGains(self):
        ...
        self.d_offset = self.setupParameter("~d_offset",
    ...
    def cbPose(self, pose_msg):
        ...
        omega_feedforward = car_control_msg.v_ref * self.velocity_to_m_per_s * pose_msg.curvature_ref * 2 * math.pi
        ...
        self.omega_max_radius_limitation = self.v_bar * car_control_msg.v_ref / self.min_radius * 2 * math.pi * self.omega_to_rad_per_s
        ...
        if math.fabs(omega) > self.omega_max:
            if self.last_ms is not None:
                self.cross_track_integral -= self.cross_track_err * dt
                self.heading_integral -= self.heading_err * dt
            car_control_msg.omega = math.copysign(self.omega_max, omega)
        else:
