...
curvature = 60                      # mm minimal turning radius
...
def pose_from_key(key):
    if key == "entrance" or key == 0:
        return np.array([lanes_length/2.0, wide_tape_width+length_red_line/2.0, -pi/2.0])
    elif key == "space 1" or key == 1:
        return np.array([lot_height-space_length/2.0, lot_width/8.0, -pi/2.0])
    elif key == "space 2" or key == 2:
        return np.array([lot_height-space_length/2.0, lot_width/8.0 + lot_width/4.0, -pi/2.0])
    elif key == "space 3" or key == 3:
        return np.array([lot_height-space_length/2.0, lot_width/8.0 + 2*lot_width/4.0, -pi/2.0])
    elif key == "space 4" or key == 4:
        return np.array([lot_height-space_length/2.0, lot_width/8.0 + 3*lot_width/4.0, -pi/2.0])
    elif key == "space 5" or key == 5:
        return np.array([space_length/2.0, lot_width/8.0 + 2*lot_width/4.0, pi/2.0])
    elif key == "space 6" or key == 6:
        return np.array([space_length/2.0, lot_width/8.0 + 3*lot_width/4.0, pi/2.0])
    elif key == "exit" or key == 7:
        return np.array([lanes_length/2.0, wide_tape_width+narrow_tape_width+3.0/2.0*length_red_line, pi/2.0])
    else:
...
if __name__ == '__main__':
    ...
    end_x, end_y, end_yaw = pose_from_key("space 1")
    if do_talking:
        print("initial pose: \n\tx = {}\n\ty = {} \n\ttheta = {}".format(
