...
namespace parking
{
    ImageUndistorter::ImageUndistorter()
        ...
        undistorted_image_pub_ = it_.advertise("/schumi/camera_node/undistorted_image", 1);
        intrinsic_ = (cv::Mat_<double>(3, 3)
                << 353.32880109224254, 0.0, 340.6187776422325, 0.0, 350.03231511108174, 256.42403699758046, 0.0, 0.0, 1.0);
        distCoeffs_ = (cv::Mat_<double>(5, 1)
                << -0.3120820098153013, 0.07611370059003515, -0.0043552007678532715, -0.00045294362665816284, 0.0);
    }
    ...
    void ImageUndistorter::imageCallback(const sensor_msgs::ImageConstPtr &msg) {
        ...
        cv::Mat distorted_img = undistorted_img.clone();

        cv::undistort(distorted_img, undistorted_img, intrinsic_, distCoeffs_);

        std::cout << undistorted_img << std::endl;
...
int main(int argc, char** argv)
{
    ...
    parking::ImageUndistorter imgund;
