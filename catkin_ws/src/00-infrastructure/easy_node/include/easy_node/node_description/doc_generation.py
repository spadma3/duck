...
def generate_easy_node_docs():
    ...
    for package_name, package_dir in packages.items():
        ...
        md += generate_from_package_info(package_info)
...
def generate_from_package_info(info):
...
def generate_configuration(config):
    ...
    choose = [_ for _ in config.parameters.values() if not _.name.startswith(COMMON_PREFIX)]
